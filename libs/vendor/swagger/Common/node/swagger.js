...
var Swagger = function () {
	...
	var errorHandler = function (req, res, error) {
	  if (error.code && error.reason)
	    ...
	    console.error(req.method + " failed for path '" + require('url').parse(req.url).href + "': " + error.stack);
	...
	this.params = params;
	this.queryParam = this.params.query;
	this.pathParam = this.params.path;
	this.bodyParam = this.params.body;
	this.formParam = this.params.form;
	this.getModels = allModels;

	this.error = error;
	this.stopWithError = stopWithError;
	this.stop = stopWithError;
	...
	this.setAppHandler = setAppHandler;
