...
(function() {
  ...
  SwaggerModelProperty = (function() {
    function SwaggerModelProperty(name, obj) {
      if ((!obj.type || typeof obj.type == 'object') && !obj.description && !obj['$ref'] && typeof obj == 'object') {
      	this.descr = [];
      	for (var key in obj) {
      		this.descr.push(new SwaggerModelProperty(key, obj[key]));
      	}
      } else {
      ...
      this.name = name;
      if (obj.type) {
      	var otype = obj.type.type || obj.type;
      }
      this.dataType = otype || obj.dataType || obj["$ref"] || 'Mixed';
    ...
    SwaggerModelProperty.prototype.toString = function(depth) {
      var req, str, linebreak = '', indent = '';
      if (!depth) depth = 0;
      if (depth) linebreak = '<br>';
      req = this.required ? 'propReq' : 'propOpt';
      str = linebreak + '<span class="propName ' + req + '" style="margin-left: ' + depth * 20 + 'px;">' + this.name + '</span> (<span class="propType">' + this.dataTypeWithRef + '</span>';
      if (!this.required) {
      ...
      if (this.descr != null && !Array.isArray(this.descr)) {
        str += ': <span class="propDesc">' + this.descr + '</span>';
      } else if (Array.isArray(this.descr)) {
      	var substr = '';
      	depth++
      	this.descr.forEach(function (d) {
      		substr += d.toString(depth)
      	})
      	str += ': <span class="propDesc">{' + substr + '}</span>'
      }
      return str;
    };
